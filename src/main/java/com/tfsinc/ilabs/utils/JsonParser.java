package com.tfsinc.ilabs.utils;
...
public class JsonParser {
    ...
    public JsonParser(final Map<Class<?>, JsonDeserializer<?>> deserializers,
        ...
        this(deserializers, serializers, null);
    }
     ...
     * @param deserializers Adapters for deserialization.
     * @param serializers   Adapters for serialization.
     * @param dateFormat    Java date format to parse date, time and timestamps.
     */
    public JsonParser(final Map<Class<?>, JsonDeserializer<?>> deserializers,
                      final Map<Class<?>, JsonSerializer<?>> serializers, final String dateFormat) {
        ...
        if (deserializers != null && deserializers.size() > 0) {
            final Iterator<Map.Entry<Class<?>, JsonDeserializer<?>>> iter1 =
                    deserializers.entrySet().iterator();
            while (iter1.hasNext()) {
                Map.Entry<Class<?>, JsonDeserializer<?>> deserializer =
                        iter1.next();
        ...
        if (serializers != null && serializers.size() > 0) {
            final Iterator<Map.Entry<Class<?>, JsonSerializer<?>>> iter2 =
                    serializers.entrySet().iterator();
            while (iter2.hasNext()) {
                Map.Entry<Class<?>, JsonSerializer<?>> serializer =
                        iter2.next();
                builder.registerTypeAdapter(
                        serializer.getKey(),
                        serializer.getValue());
